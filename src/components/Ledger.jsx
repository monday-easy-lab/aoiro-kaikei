        ...
        {filtered.length === 0 ? (
          ...
          <>
            ...
            <div className="ledger-cards">
              {filtered.map((e) => (
                <div key={e.id} style={{
                  padding: "12px 14px", marginBottom: 8, background: "#f8fafc",
                  borderRadius: 10, border: "1px solid #e2e8f0",
                }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                    <span style={{ fontSize: 12, color: "#6b7280" }}>{e.date}</span>
                    <span style={{ fontSize: 18, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>¥{fmt(e.amount)}</span>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#1e293b", marginBottom: 2 }}>
                    {getAccountName(e.debitCode)} → {getAccountName(e.creditCode)}
                  </div>
                  {e.memo && <p style={{ margin: "4px 0 8px", fontSize: 12, color: "#6b7280" }}>{e.memo}</p>}
